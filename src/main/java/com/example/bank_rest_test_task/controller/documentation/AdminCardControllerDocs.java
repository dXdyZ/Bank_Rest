...
public interface AdminCardControllerDocs {
    ...
    ResponseEntity<Page<CardDto>> getUserCards(@Positive(message = "Id must not be less than zero") @PathVariable Long id,
    ...
    ResponseEntity<Page<CardDto>> getCardsByUserName(@NotBlank(message = "Username must be not empty") @RequestParam String username, @ParameterObject @PageableDefault(size = 6) Pageable pageable);
    ...
    ResponseEntity<Page<CardDto>> getAllCards(@ParameterObject @PageableDefault(size = 6, sort = "validityPeriod") Pageable pageable);
}
